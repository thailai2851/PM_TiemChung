...
$(document).ready(function () {
    ...
    $('#formUpdate').submit(function (event) {
        ...
        if (!form.checkValidity()) {
            ...
            $.ajax({
                url: '/DanhMuc/DM_QuocGia/update', // Đường dẫn đến action xử lý form
                method: 'POST',
                data: formData,
    ...
    $(document).on('click', '#btnDanger', function () {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuocGia/changeActive",
            data: "id=" + idModel,
            success: function (response) {
                if (response.statusCode == 200) {
...
function getRowTable(data) {
    ...
    <td class="text-center MaQuocGia">${data.maQuocGia == null ? "" : data.maQuocGia}</td>
    <td class="text-start TenQuocGia">${data.tenQuocGia == null ? "" : data.tenQuocGia}</td>
...
function changePage(pageNumber) {
    ...
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_QuocGia/changePage",
...
function getSessionColumnShow() {
    ...
    if (sessionStorage.getItem(sessionName)) {
        ...
        var defaultValue = [
            {
                columnName: "MaQuocGia",
                value: true,
            },
            {
                columnName: "TenQuocGia",
                value: true,
            },
        ];
...
function searchWithKeyword() {
    ...
    if (key == "") {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuocGia/api/getModelsWithNumberPage",
            success: function (response) {
                dataResponse = [];
                hideProgress();
                updateDataOfTable(response.result);
        ...
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuocGia/api/searchWithKeyword",
...
function showModal(id) {
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_QuocGia/showModal",
        data: "id=" + id,
        success: function (response) {
