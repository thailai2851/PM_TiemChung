...
$(document).ready(function () {
    ...
    $('#formUpdate').submit(function (event) {
        ...
        if (!form.checkValidity()) {
            ...
            $.ajax({
                url: '/DanhMuc/DM_DanToc/update', // Đường dẫn đến action xử lý form
                method: 'POST',
                data: formData,
    ...
    $(document).on('click', '#btnDanger', function () {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_DanToc/changeActive",
            data: "id=" + idModel,
            success: function (response) {
                if (response.statusCode == 200) {
...
function getRowTable(data) {
    ...
    <td class="text-center MaDanToc">${data.maDanToc == null ? "" : data.maDanToc}</td>
    <td class="text-start TenDanToc">${data.tenDanToc == null ? "" : data.tenDanToc}</td>
    <td class="text-start TenGoiKhac">${data.tenGoiKhac == null ? "" : data.tenGoiKhac}</td>
    <td class="text-start DiaBanCuTru">${data.diaBanCuTru == null ? "" : data.diaBanCuTru}</td>
...
function changePage(pageNumber) {
    ...
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_DanToc/changePage",
...
function getSessionColumnShow() {
    ...
    if (sessionStorage.getItem(sessionName)) {
        ...
        var defaultValue = [
            ...
            {
                columnName: "TenDanToc",
                value: true,
            },
            {
                columnName: "TenGoiKhac",
                value: true,
            },
            {
...
function searchWithKeyword() {
    ...
    if (key == "") {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_DanToc/api/getModelsWithNumberPage",
            success: function (response) {
                dataResponse = [];
                hideProgress();
                updateDataOfTable(response.result);
        ...
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_DanToc/api/searchWithKeyword",
...
function showModal(id) {
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_DanToc/showModal",
        data: "id=" + id,
        success: function (response) {
