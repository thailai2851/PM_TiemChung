...
namespace PM_TiemChung.Controllers
{
    ...
    public class DM_ThoiGianController : Controller
    {
        private IThoiGianServices _services;
        private ICompositeViewEngine _viewEngine;

        public DM_ThoiGianController(IThoiGianServices dmIcdYhctServices, ICompositeViewEngine viewEngine)
        {
            _services = dmIcdYhctServices;
            _viewEngine = viewEngine;
        }
        [HttpGet]
        public async Task<IActionResult> ThoiGian()
        {
        ...
        public async Task<IActionResult> showModal(long id)
        {
            ...
            PartialViewResult partialViewResult = PartialView("FormThoiGian", model == null ? new DmThoiGian() : model);
            string viewContent = ConvertViewToString(ControllerContext, partialViewResult, _viewEngine);
            return Ok(new
            {
                view = viewContent,
                title = model == null ? "Thêm thời gian" : "Chỉnh sửa thời gian"
            });
        }
        [HttpPost("update")]
        // thêm và cập nhập
        public async Task<IActionResult> update(DmThoiGianMap modelMap)
        {
            var result = await _services.UpdateThoiGian(modelMap);
            return Ok(result);
        }
