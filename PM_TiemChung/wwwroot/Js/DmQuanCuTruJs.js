...
$(document).ready(function () {
    ...
    $('#formUpdate').submit(function (event) {
        ...
        if (!form.checkValidity()) {
            ...
            $.ajax({
                url: '/DanhMuc/DM_QuanCuTru/update', // Đường dẫn đến action xử lý form
                method: 'POST',
                data: formData,
    ...
    $(document).on('click', '#btnDanger', function () {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuanCuTru/changeActive",
            data: "id=" + idModel,
            success: function (response) {
                if (response.statusCode == 200) {
...
function getRowTable(data) {
    ...
    <td class="text-center MaQuan">${data.maQuan == null ? "" : data.maQuan}</td>
    <td class="text-start TenQuan">${data.tenQuan == null ? "" : data.tenQuan}</td>
    <td class="text-start Idtinh">${data.idtinhNavigation.tenTinh == null ? "" : data.idtinhNavigation.tenTinh}</td>
...
function changePage(pageNumber) {
    ...
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_QuanCuTru/changePage",
...
function searchWithKeyword() {
    ...
    if (key == "") {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuanCuTru/api/getModelsWithNumberPage",
            success: function (response) {
                dataResponse = [];
                hideProgress();
                updateDataOfTable(response.result);
        ...
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_QuanCuTru/api/searchWithKeyword",
...
function showModal(id) {
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_QuanCuTru/showModal",
        data: "id=" + id,
        success: function (response) {
