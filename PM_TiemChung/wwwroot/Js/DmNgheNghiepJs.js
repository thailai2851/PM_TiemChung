...
$(document).ready(function () {
    ...
    $('#formUpdate').submit(function (event) {
        ...
        if (!form.checkValidity()) {
            ...
            $.ajax({
                url: '/DanhMuc/DM_NgheNghiep/update', // Đường dẫn đến action xử lý form
                method: 'POST',
                data: formData,
    ...
    $(document).on('click', '#btnDanger', function () {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_NgheNghiep/changeActive",
            data: "id=" + idModel,
            success: function (response) {
                if (response.statusCode == 200) {
...
function getRowTable(data) {
    ...
    <td class="text-center MaNgheNghiep">${data.maNgheNghiep == null ? "" : data.maNgheNghiep}</td>
    <td class="text-start TenNgheNghiep">${data.tenNgheNghiep == null ? "" : data.tenNgheNghiep}</td>
...
function changePage(pageNumber) {
    ...
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_NgheNghiep/changePage",
...
function getSessionColumnShow() {
    ...
    if (sessionStorage.getItem(sessionName)) {
        ...
        var defaultValue = [
            {
                columnName: "MaNgheNghiep",
                value: true,
            },
            {
                columnName: "TenNgheNghiep",
                value: true,
            },
        ];
...
function searchWithKeyword() {
    ...
    if (key == "") {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_NgheNghiep/api/getModelsWithNumberPage",
            success: function (response) {
                dataResponse = [];
                hideProgress();
                updateDataOfTable(response.result);
        ...
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_NgheNghiep/api/searchWithKeyword",
...
function showModal(id) {
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_NgheNghiep/showModal",
        data: "id=" + id,
        success: function (response) {
