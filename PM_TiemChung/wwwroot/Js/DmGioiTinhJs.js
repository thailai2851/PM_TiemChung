...
$(document).ready(function () {
    ...
    $('#formUpdate').submit(function (event) {
        ...
        if (!form.checkValidity()) {
            ...
            $.ajax({
                url: '/DanhMuc/DM_GioiTinh/update', // Đường dẫn đến action xử lý form
                method: 'POST',
                data: formData,
    ...
    $(document).on('click', '#btnDanger', function () {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_GioiTinh/changeActive",
            data: "id=" + idModel,
            success: function (response) {
                if (response.statusCode == 200) {
...
function getRowTable(data) {
    ...
    <td class="text-center MaGioiTinh">${data.maGioiTinh == null ? "" : data.maGioiTinh}</td>
    <td class="text-start TenGioiTinh">${data.tenGioiTinh == null ? "" : data.tenGioiTinh}</td>
...
function changePage(pageNumber) {
    ...
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_GioiTinh/changePage",
...
function getSessionColumnShow() {
    ...
    if (sessionStorage.getItem(sessionName)) {
        ...
        var defaultValue = [
            {
                columnName: "MaGioiTinh",
                value: true,
            },
            {
                columnName: "TenGioiTinh",
                value: true,
            },
        ];
...
function searchWithKeyword() {
    ...
    if (key == "") {
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_GioiTinh/api/getModelsWithNumberPage",
            success: function (response) {
                dataResponse = [];
                hideProgress();
                updateDataOfTable(response.result);
        ...
        $.ajax({
            type: "post",
            url: "/DanhMuc/DM_GioiTinh/api/searchWithKeyword",
...
function showModal(id) {
    $.ajax({
        type: "post",
        url: "/DanhMuc/DM_GioiTinh/showModal",
        data: "id=" + id,
        success: function (response) {
