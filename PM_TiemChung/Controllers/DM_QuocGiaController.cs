...
namespace PM_TiemChung.Controllers
{
    ...
    public class DM_QuocGiaController : Controller
    {
        private IQuocGiaServices _services;
        private ICompositeViewEngine _viewEngine;

        public DM_QuocGiaController(IQuocGiaServices dmIcdYhctServices, ICompositeViewEngine viewEngine)
        {
            _services = dmIcdYhctServices;
            _viewEngine = viewEngine;
        }
        [HttpGet]
        public async Task<IActionResult> QuocGia()
        {
        ...
        public async Task<IActionResult> showModal(long id)
        {
            ...
            PartialViewResult partialViewResult = PartialView("FormQuocGia", model == null ? new DmQuocGium() : model);
            string viewContent = ConvertViewToString(ControllerContext, partialViewResult, _viewEngine);
            return Ok(new
            {
                view = viewContent,
                title = model == null ? "Thêm quốc gia" : "Chỉnh sửa quốc gia"
            });
        }
        [HttpPost("update")]
        // thêm và cập nhập
        public async Task<IActionResult> update(DmQuocGiumMap modelMap)
        {
            var result = await _services.UpdateQuocGia(modelMap);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> getListQuocGia()
        {
            var result = await _services.getListQuocGia();
            return Ok(result);
        }
