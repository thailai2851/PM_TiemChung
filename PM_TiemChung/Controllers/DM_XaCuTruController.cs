...
namespace PM_TiemChung.Controllers
{
    ...
    public class DM_XaCuTruController : Controller
    {
        private IXaCuTruServices _services;
        private ICompositeViewEngine _viewEngine;

        public DM_XaCuTruController(IXaCuTruServices dmIcdYhctServices, ICompositeViewEngine viewEngine)
        {
            _services = dmIcdYhctServices;
            _viewEngine = viewEngine;
        }
        [HttpGet]
        public async Task<IActionResult> XaCuTru()
        {
        ...
        public async Task<IActionResult> showModal(long id)
        {
            ...
            PartialViewResult partialViewResult = PartialView("FormXaCuTru", model == null ? new DmXaCuTru() : model);
            string viewContent = ConvertViewToString(ControllerContext, partialViewResult, _viewEngine);
            return Ok(new
            {
                view = viewContent,
                title = model == null ? "Thêm xã cư trú" : "Chỉnh sửa xã cư trú"
            });
        }
        [HttpPost("update")]
        // thêm và cập nhập
        public async Task<IActionResult> update(DmXaCuTruMap modelMap)
        {
            var result = await _services.UpdateXaCuTru(modelMap);
            return Ok(result);
        }
        ...
        [HttpPost("getListXaCuTru")]
        public async Task<IActionResult> getListXaCuTru()
        {
            var result = await _services.getListXaCuTru();
            return Ok(result);
        }
