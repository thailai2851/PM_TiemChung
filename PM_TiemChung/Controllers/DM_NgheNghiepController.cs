...
namespace PM_TiemChung.Controllers
{
    ...
    public class DM_NgheNghiepController : Controller
    {
        private INgheNghiepServices _services;
        private ICompositeViewEngine _viewEngine;

        public DM_NgheNghiepController(INgheNghiepServices dmIcdYhctServices, ICompositeViewEngine viewEngine)
        {
        ...
        public async Task<IActionResult> showModal(long id)
        {
            ...
            PartialViewResult partialViewResult = PartialView("FormNgheNghiep", model == null ? new DmNgheNghiep() : model);
            string viewContent = ConvertViewToString(ControllerContext, partialViewResult, _viewEngine);
            return Ok(new
            {
                view = viewContent,
                title = model == null ? "Thêm nghề nghiệp" : "Chỉnh sửa nghề nghiệp"
            });
        }
        [HttpPost("update")]
        // thêm và cập nhập
        public async Task<IActionResult> update(DmNgheNghiepMap modelMap)
        {
            var result = await _services.UpdateNgheNghiep(modelMap);
            return Ok(result);
        }
        ...
        [HttpPost("getListNgheNghiep")]
        public async Task<IActionResult> getListNgheNghiep()
        {
            var result = await _services.getListNgheNghiep();
            return Ok(result);
        }
